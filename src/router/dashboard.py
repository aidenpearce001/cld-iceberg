...
def main():
    ...
    if len(top_country) > 0:  # If found at least an item
        ...
        url_based = ['special_char', 'url_depth', 'use_http', 'redirection', 'url_length', 'time_get_redirect',
                     'prefix_suffix', 'tiny_url', 'port_in_url', 'have_at', 'http_in_domain', 'have_ip', 'Punny_Code']
        domain_based = ['same_asn', 'domain_lifespan',
                        'domain_timeleft', 'dns_record', 'trusted_ca']
        content_based = ['iframe', 'Web_Forwards', 'mouse_over', 'Right_Click',
                         'fromCharCode', 'ActiveXObject', 'escape', 'eval', 'atob', 'unescape']
        percent_list = []
